...
for outfilename,folder_suffix in files:
    ...
    for filename in os.listdir('mubin'+folder_suffix):
        ...
        for actor in dropactor_objs:
            ...
            for node in actor.findall('./LinksToObj/value') :
                if ( node.findall('./DefinitionName')[0].text == 'Reference' or 
                    node.findall( '.DefinitionName' )[0].text == 'BasicSig' or 
                    node.findall( '.DefinitionName' )[0].text == 'Create' or 
                    node.findall( '.DefinitionName' )[0].text == 'BasicSigOnOnly' ) : 
                    references.append( node.attrib['DestUnitHashId'] )
            ...
            objects[name]['locations'][HashId] = {}
            
            objects[name]['locations'][HashId]['coords'] = [round(coords[0],2), round(coords[-1],2)]

            # Another place to store it for us to use later in our own loop
            objects[name]['locations'][HashId]['name'] = name 

            firstKorokFlower = False
            if ( name == 'Obj_Plant_Korok_A_01' ) : 

                if ( actor.find( './_Parameters' ) is not None and 'IsNoAppearEffect' in actor.find( './_Parameters' ).attrib ) :
                    firstKorokFlower = actor.find( './_Parameters' ).attrib["IsNoAppearEffect"]
            
                objects[name]['locations'][HashId]['firstKorokFlower'] = firstKorokFlower

            if len( references ) != 0 :
                objects[name]['locations'][HashId]['references'] = references
    ...
    #        if name not in objects:
    #            objects[name] = {'display_name':nice_name, 'locations':{}}
    #        objects[name]['locations'][0]['coords'] += [(round(x,2),round(z,2)) for x,y,z in chunk_objects[name]]
